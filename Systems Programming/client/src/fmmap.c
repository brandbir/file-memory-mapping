...
fileloc_t temp_file_mapped;
off_t file_mapped_off;

void *rmmap(fileloc_t location, off_t offset)
{
	//initialising the the temporary variable for holding the file details for future reference
	temp_file_mapped.ipaddress = location.ipaddress;
	temp_file_mapped.pathname = location.pathname;
	temp_file_mapped.port = location.port;
	file_mapped_off = offset;
	...
	//blocking while andrew was connected before hand
	//kif naghmlu unmap ihallina naqraw
...
ssize_t mread(void *addr, off_t offset, void *buff, size_t count)
{
	...
	//Checking if we need to update memory mapped contents before performing  write
	char update_code[7] = "       ";
	bzero(update_code, strlen(update_code) + 1);
	sleep(1);
